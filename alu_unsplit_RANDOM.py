...
def RP_intervals(max_length):
    original_list = list(range(1, int(max_length + 1)))

    # Randomly shuffle the list
    random.shuffle(original_list)
    half_max = max_length // 2
    # Split the shuffled list into two substrings
    substring1 = original_list[:half_max]
    substring2 = original_list[half_max:]

    return substring1, substring2

def create_RPseq(seq, positions):
    seq_RP = ''.join(seq[i-1] for i in positions)
    return seq_RP

phase_A, phase_B = (RP_intervals(int(args.bin_size)))

print("phaseA = " + str(phase_A))
print("phaseB = " + str(phase_B))
...
def alignment_protopairs(pandas_blast):
    with open(str("Protopairs_unsplit_RP1_96nt.tsv"), "a") as output_table:
        for row in pandas_blast.itertuples():
            ...
            if alu_mask_check.empty:
                ...
                RP1Ag = create_RPseq(genomic_ptrans, phase_A)
                RP1Bg = create_RPseq(genomic_ptrans, phase_B)

                RP1Ate = create_RPseq(alu_bin_seq, phase_A)
                RP1Bte = create_RPseq(alu_bin_seq, phase_B)

                homology_RP1A = calculate_homology(str(RP1Ag), str(RP1Ate))
                homology_RP1B = calculate_homology(str(RP1Bg), str(RP1Bte))

                print(f"{homology_RP1A:.2f} \t {homology_RP1B:.2f}", file=output_table)
    output_table.close()


print("Calculating homology...")
#rm_file(str('Protopairs_unsplit_RP1.tsv'))
dictio = str.maketrans({'A': 'G', 'T': 'C'})  # define the purine code
